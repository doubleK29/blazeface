...
def model_info(model, verbose=False, img_size=320, name=None, fuse=False):
    ...
    if verbose:
        print('%5s %40s %9s %12s %20s' % ('layer', 'name', 'gradient', 'parameters', 'shape'))
    ...
    print((f"{name} " if name else "") + f"Model Summary: "
                                      f"{len(list(model.modules()))} layers, {n_p} parameters, {n_g} gradients{fs}")
