...
def fitness(x):
    ...
    w = [0., 0.0, 0.2, 0.8]  # weights for [P, R, mAP@0.5, mAP@0.5:0.95]
    return (x[:, :4] * w).sum(1)


def check_model(model_class, args, inp, expected_oup):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = model_class(*args).to(device)
    out = model(inp)
    try:
        if isinstance(expected_oup, list):
            for i in range(len(expected_oup)):
                assert out[i].shape == expected_oup[i].shape, print(
                    f"Expected output shape: {expected_oup[i].shape}.\nGot: {out[i].shape}. In dimension {i + 1}th")
            print("Check Successfully!")
        else:
            assert out.shape == expected_oup.shape, print(
                f"Expected output shape: {expected_oup.shape}.\nGot: {out.shape}.")
    except AttributeError:
        raise AttributeError(f"Bad arguments! {traceback.print_exc()}")
...
def print_mutation(hyp, results, yaml_file='hyp_evolved.yaml', bucket=''):
    # Print mutation results to evolve.txt (for use with trainer.py --evolve)
