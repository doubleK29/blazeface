...
# path2train = yaml.load(open("../config/data.widerface.yaml"), Loader=yaml.FullLoader).get("ori_train")
# aa = WiderFaceDetection(path2train)
...
path = "../config/data.combine.yaml"
yl = yaml.load(open(path), Loader=yaml.FullLoader)

dir_image = yl.get("ori_train")
size = 0
num = 0
wf = open("../data/combine/path.txt", "w")
for Folderpath in tqdm(dir_image):
    for path, dirs, files in os.walk(Folderpath):
        for f in files:
            json_path = os.path.join(path, f[:-4] + ".json")
            if f.endswith(".jpg") and os.path.exists(json_path):
                wf.write(f"{json_path}" + "\n")
