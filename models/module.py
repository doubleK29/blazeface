...
from logger.visualization import color_list
...
class autoShape(nn.Module):
    ...
    def autoshape(self):
        ...
        )  # models already converted to models.autoshape()
        return self
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), kernel_size=1):
        ...
        self.m = nn.ModuleList(
            nn.Conv2d(
                x, self.no * self.na, kernel_size, padding=1 if kernel_size > 1 else 0
            )
            for x in ch
        )  # output conv
